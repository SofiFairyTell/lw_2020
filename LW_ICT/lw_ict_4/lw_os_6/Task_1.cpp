...
struct SLP_msg
{
	...
	WCHAR username[20] = L""; //имя отправителя
	WCHAR text[10] = L"";		//текст сообщения
};
#pragma pack()
...
BOOL OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct)
{
	...
	hList = CreateWindowEx(0, TEXT("ListBox"), NULL,
		WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | LBS_NOINTEGRALHEIGHT | LBS_DISABLENOSCROLL, 10, 100, 600,
	...
	if (BN_CLICKED == codeNotify)
	{
		...
		case IDC_BUTTON:
		{
			...
			for (int i = 0; i < num; i ++)
			{
				if ((i + 1) < num)
				{
					StringCchCopyNW(msg.text, _countof(msg.text), &text[i * (_countof(msg.text) - 1)], _countof(msg.text));
				}
				else
				{
					StringCchCopyW(msg.text, _countof(msg.text), &text[i * (_countof(msg.text) - 1)]);
				}
				msg.numberfrag = i;
...
unsigned __stdcall ThreadFunc(void* lpParameter)
{
	...
	for (;;)
	{
		...
		if (SOCKET_ERROR != n) // сообщение успешно получено
		{
			...
			{			
				StringCchCatNW(text, _countof(text), msgt.text, _countof(msgt.text));
				//StringCchCopyNW(text, _countof(text), msgt.text, _countof(msgt.text));
			}
			else
			{
				StringCchCatW(text, _countof(text), msgt.text);
				//StringCchCopyW(text, _countof(text), msgt.text);
			}
			ZeroMemory(msgt.text, sizeof(msgt.text));
			count++;
			if (count == num)
			{
